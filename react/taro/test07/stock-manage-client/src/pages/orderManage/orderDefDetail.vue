  ...
  <div class="conBigDiv">
    ...
    <div class="info">
      <div class="info-top">
        <div class="infot-table">
          ...
          <div class="tr">
            ...
            <div class="td" v-if="Number(result.startOrderTotailWeight)">{{result.startOrderTotailWeight}}-{{result.orderTotalWeight}}</div>
            ...
            <div class="td handle">
              <el-button type="primary" size="mini" v-if="result.orderStatus == 1" @click="upAudit">提交审核</el-button>
              <el-button type="warning" style="margin-top: 10px" size="mini" v-if="result.orderStatus == 1"
                         @click="backOrder">退回
              </el-button>
            </div>
          </div>
          ...
          <div class="tr" style="border: none">
            <el-col :span="8">
              <el-steps :active="stepsActive" finish-status="success" class="steps-list" >
                <el-step title="步骤 1" size="mini" v-for="(item,index) in result.perFroms" :key="index">
                  <div slot="title">{{item.prcessName}}</div>
                  <div slot="description">
                    <p v-for="(it,ind) in item.processTypeFroms" :key="ind">
                      {{it.operateName}}
                    </p>
                  </div>
                </el-step>
      ...
      <div class="info-middle">
        <div class="info-item-wrap" v-for="(iitem,index) in result.factoryProductFroms" :key="index">
            <p class="infom-tit">
                ...
                <div class="iitem-right">
                  ...
                  <div class="order-item">
                    <div class="order-tr" v-for="(it,index) in item.orderPeoductPerties" :key="index">
                      ...
                      <div class="otd" v-if="it.styleItem.name">{{it.styleItem.value  || '无'}}</div>
          ...
          <div class="suit" v-for="(item,ind) in iitem.suitProductFroms" :key="ind">
            ...
            <div class="suit-info">
              <div class="infom-info suiti-item"  v-for="(it, ix) in item.suitProdctPeyFroms" :key="ix">
                <div class="infom-item">
                  <div class="pro-item iitem-left">
                    <div class="pro-imgs">
  ...
  export default {
    ...
    methods: {
      created_fun() {
        let self = this;
        this.$axios
          .post(this.$portMain + "/orderManage/selectExtractOrderDetailByOrderNo?orderNo=" + this.orderNo)
          .then(res => {
            let stepAc = ''
            ...
            if(stepAc) {
              ...
              res.data.data.factoryProductFroms.forEach(iitem => {
                ...
                iitem.orderProperties.forEach(item => {
                  ...
                  item.orderPeoductPerties.forEach(it => {
                    //  尚臻品下 计算单个产品重量
                    if(item.suit) {
                      it.standardGold = it.suitStandardGold ? it.suitStandardGold.toString() : it.standardGold.toString()
                      if(!it.standardGold.split('-')[1]) {
                        it.standardGold = `${Number(it.standardGold)}-${Number(it.standardGold)}`
                      }
                      if(it.standardGold.split('-')[1]) {
                          oneStart += Number(it.standardGold.split('-')[0]) * Number(it.orderSum)
                          oneEnd += Number(it.standardGold.split('-')[1]) * Number(it.orderSum)
                  ...
                  if(item.suit) {
                    if(!item.weightShow) {
                      ...
                      iitem.allTotalWeight = `${oneStart.toFixed(2)}-${oneEnd.toFixed(2)}`
                    }else {
                ...
                if(iitem.suitProductFroms) {
                  iitem.suitProductFroms.forEach(it => {
                    it.standardGold = 0, it.innerWeightStart = 0, it.innerWeightEnd = 0, it.orderSum = 0, it.allWeight = 0
                    ...
                    it.suitProdctPeyFroms.forEach(iit => {
                      ...
                      iit.appearance.forEach(iiit => {
                        if (iiit.name === `diameterLengthStart` && iiit.value) {
                          ...
                          iiit.styleItem = { name: `圈口`, value: iit.diameterLength } 
                        }
                      ...
                      it.allWeight += Number(iit.suitStandardGold) * Number(iit.orderSum)
              ...
              res.data.data.factoryProductFroms.forEach(item => {
                  item.allTotalStart = 0, item.allTotalEnd = 0, item.allTotal = 0, item.faWeight = 0
                if(item.suitProductFroms) {
                  ...
                  if(item.allTotalWeight.toString().split('-')[1]) {
                      item.allTotalStart =  (Number(item.allTotalWeight.toString().split('-')[0]) + Number(item.faWeight))
                      item.allTotalEnd =   (Number(item.allTotalWeight.toString().split('-')[1]) + Number(item.faWeight))  
                  }
                }else {
                  item.allTotalWeight = item.allTotalWeight
                }
                if(item.suitProductFroms && item.allTotalWeight.toString().split('-')[1]) {
                  item.allTotalWeight = `${item.allTotalStart.toFixed(2)}-${item.allTotalEnd.toFixed(2)}`
            ...
            res.data.data.startOrderTotailWeight = Number(res.data.data.startOrderTotailWeight).toFixed(2)
            res.data.data.orderTotalWeight = Number(res.data.data.orderTotalWeight).toFixed(2)
            if(Number(res.data.data.startOrderTotailWeight) === Number(res.data.data.orderTotalWeight)) {
      ...
      upAudit() {
        this.$axios.post(this.$portMain + '/orderManage/updataOrdersByOrderNo?orderNo=' + this.orderNo).then(res => {
      ...
      backOrder() {
        this.$prompt('请输入退回原因', '提示', {
          ...
          this.$axios.get(this.$portMain + '/orderManage/backOrdersRemarks', params).then(res => {
            if(res.data.code === 200) {
