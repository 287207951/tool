    ...
    <div class="info" v-loading="webLoadingTF">
        ...
        >
          <span>
            <div style="display:flex">
              <div style="width:25%" v-if="findAllData.number">入饰单号：{{findAllData.number}}</div>
            ...
            <div style="display:flex">
              <div style="width:32%">
                ...
                >
                  <el-table-column align="center" type="index" label="序号" width="60"></el-table-column>
                  <el-table-column align="center" prop="conditi" label="种类">
                    <!-- <template slot-scope="scope">
                      <el-input v-model="scope.row.conditi" placeholder size="small"></el-input>
                    </template>-->
                  </el-table-column>
                  <el-table-column
                    prop="marketingCategoryName"
                  ...
                  <el-table-column prop="weight" align="center" label="实收重量">
                    <!-- <template slot-scope="scope">
                      <el-input v-model="scope.row.weight" placeholder size="small"></el-input>
                    </template>-->
                  </el-table-column>
                </el-table>
              </div>
              <div style="width:68%">
                ...
                >
                  ...
                  <el-table-column prop="ceditMoney" align="center" label="金额">
                    <!-- <template slot-scope="scope">
                      <div :title="scope.row.ceditMoney">
                        <el-input-number
                          class="width62"
                          :controls="false"
                  ...
                  <!-- <el-table-column prop="address" align="center" label="操作" width="100">
                    <template slot-scope="scope">
                      ...
                      <span class="redColor" @click="delDataOne(scope.$index, tableRightData)">删除</span>
                    </template>
                  </el-table-column>-->
                </el-table>
              </div>
      ...
      <div v-if="orderHeader.ownOrderStatus == 1 || orderHeader.ownOrderStatus == 3">
        <div class="oneClass">
          ...
          >
          <el-checkbox v-model="checkTF"  v-show="false"></el-checkbox>
            <el-button
              size="small"
              @click="oneKeyAddOne"
              :loading="btnLoadingTF"
                      ...
                      <div class="pro-right">
                        ...
                        <el-row style="height: 35px;">
                          ...
                          <el-col
                            ...
                            v-if="orderHeader.ownOrderStatus == 1 || orderHeader.ownOrderStatus == 3"
                          >
                            <span style="margin-left: -19px;">实收金重(g)：</span>
                            <el-input-number
                        ...
                        <el-row style="height: 35px;">
                          ...
                          >
                            ...
                            <el-input-number
                              ...
                              :min="0"
                              size="small"
                              :disabled="orderHeader.receiableNum<=0"
  ...
  methods: {
    created_fun() {
      ...
      this.$api.formdataPostFun(
        this.$portMain + "/stockReceiv/selectProductPer",
        params,
        res => {
    ...
    oneKeyAddTwo() {
      ...
      if (subData.length === 0) {
        ...
        this.$axios
          .post(self.$portMain + "/stockReceiv/find", params)
          .then(function(res) {
            self.btnLoadingTF = false;
            if (!res) {
              self.$message.error(
                "提交数据有误，数量不能为小数，请修改后再重新提交"
              );
            }
            if (res.data.code == 200) {
    ...
    changeNumTwo(elem){
      if(elem.number){
        elem.number = parseInt(elem.number)
        if(elem.number<0){
          elem.number = 0
        }
      }
      setTimeout(()=>{
        this.checkTF = !this.checkTF 
        this.$forceUpdate()
      },100)
    ...
    oneKeyAddOne() {
      ...
      this.$confirm("此操作将终止收货, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.btnLoadingTF = true;
          this.$axios
            .get(this.$portMain + "/stockReceiv/updateStockType", params)
            .then(function(res) {
              if (res.data.code == 200) {
    ...
    viewFun(elem) {
      ...
      this.$axios
        .get(this.$portMain + "/stockReceiv/findRS", params)
        .then(function(res) {
          if (res.data.code == 200) {
            self.tableLeftData = res.data.data.ProStockDetail;
            self.tableRightData = res.data.data.OutstoreCedit;
    ...
    submitFunOne() {
      ...
      self.tableRightData.forEach(ielem => {
        ...
        self.options.forEach(jelem => {
          if (ielem.ceditWayName === jelem.ceditWayName) {
            ielem.ceditWayId = jelem.ceditWayId;
          }
        });
      });
      self.tableLeftData.forEach(item => {
      ...
      setTimeout(function() {
        ...
        self.$axios
          .post(self.$portMain + "/stockReceiv/addStock", params)
          .then(function(res) {
            if (res.data.code == 200) {
              self.btnLoadingTF = false;
              ...
              setTimeout(function() {
                ...
                // self.LODOP.PREVIEW();
                self.remakePrintData(res.data.data);
    ...
    //组装打印数据
    remakePrintData(number) {
      this.decorationAllData = {
        receiver: "深圳尚金缘金库",
        receCode: this.orderHeader.factoryOrderNo,
        rCode: number,
        customerName: this.orderHeader.factoryName,
        checkTime: this.orderHeader.createTime,
        note: this.orderHeader.note,
        skuCeditGroups: this.tableLeftData,
        outstoreCedits: this.tableRightData
      };
