D<template>
  <div class="conBigDiv" v-loading="loadingState">
    ...
    <div>
      ...
      >
        <el-table-column prop="rCode" label="入库单据号"></el-table-column>
        ...
        <!-- <el-table-column label="工单号">
          <template slot-scope="scope">
            <span v-if="scope.row.workSatue != '_allotputExhibi'">{{scope.row.orderCode}}</span>
            <span v-else>----</span>
          </template>
        </el-table-column>-->
        <el-table-column prop="receCode" label="调拨单"></el-table-column>
        ...
        <el-table-column prop label="操作人">
          <template slot-scope="scope">
            <span v-if="scope.row.billStatue == 'T'">{{scope.row.checkName}}</span>
            <span v-else>{{scope.row.reviewPersonName}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="crearorTime" label="操作时间" width="180"></el-table-column>
        <el-table-column prop="crearorTime" label="操作状态">
          <template slot-scope="scope">
            <span v-if="scope.row.billStatue == 'Y' || scope.row.billStatue == 'D'">已完成</span>
            <span class="orange" v-else-if="scope.row.billStatue == 'N'">待确认</span>
            <span class="red" v-else-if="scope.row.billStatue == 'T'">已驳回</span>
