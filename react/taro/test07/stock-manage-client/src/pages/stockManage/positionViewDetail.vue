        ...
        <el-row class="palletDiv">
          <el-col :span="5">
            <img
              ...
              @click="showPic($portImg+editSkuData.pic)"
            />
            <img
              v-else
              :src="editSkuData.pic? $portImg+editSkuData.pic:'../../../../static/images/img-noimg.png'"
              style="height:120px; width:150px;"
            />
          </el-col>
          ...
          <el-col :span="5" class="palletTxtTwo">编号：{{editSkuData.factoryNumber}}</el-col>
          ...
          <el-col :span="5" class="palletTxtOne">库存数量(件)：{{editSkuNumData.amount}}</el-col>
          <el-col :span="5" class="palletTxtOne">最高容量(件)：{{editSkuNumData.upStore}}</el-col>
          <el-col :span="5" class="palletTxtOne">最低容量(件)：{{editSkuNumData.downStore}}</el-col>
          ...
          <el-col :span="4" class="palletTxtOne">
            <el-button type="primary" @click="addReplenishCart" size="mini">添加到补货购物车111</el-button>
          </el-col>
        </el-row>
      </div>
    </el-dialog>
  ...
  methods: {
    ...
    editAddPro() {
      ...
      if (!self.selectProId) {
        ...
        this.$api.formdataPostFun(
          self.$portMain + "/bizPlate/placeSku",
          params,
          res => {
            const obj = {};
            let _i, _j;
            self.positionData.sark.forEach((ielem, i) => {
              ielem.plist.forEach((jelem, j) => {
                if (jelem.id === self.editSkuNumData.id) {
                  _i = i;
                  _j = j;
                  obj.skuNum = self.selectProId;
                  obj.upStore = 0;
                  obj.backgroundColor = "#E6E6E6";
                }
              });
            });
            this.$set(
              this.positionData.sark[_i].plist,
              _j,
              Object.assign(this.positionData.sark[_i].plist[_j], obj)
            );
            self.$message.success(res.msg);
            self.addProTF = false;
            // self.tabeDisplayTF = false;
    ...
    editTabFun(elem) {
      ...
      if (elem.skuNum) {
        ...
        this.$axios
          .get(self.$portMain + "/product/findSku", params)
          .then(function(res) {
            if (res.data.code == 200) {
        ...
        // console.log(self.editSkuNumData)
      }
    },
    //分页事件开始
