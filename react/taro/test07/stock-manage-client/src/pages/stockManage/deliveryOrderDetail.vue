  ...
  <div class="conBigDiv" ref="repleOrder">
              ...
              >
                ...
                <el-col :span="6">
                  <el-col :span="8" class="txtCenter">
                    <el-button
                      size="small"
                      v-if="!ielem.backup && ielem.newTF"
                      @click="addBackup(ielem)"
                    >添加备注</el-button>
                    <el-tooltip :content="ielem.backup" placement="top" v-if="ielem.backup">
          ...
          <span>
            <div>
              <el-row class="titTwoBao">
                ...
                <span class="txtRight30">净重：{{viewBaoHead.totailGoldWeight}}</span>
                <span>毛重：{{viewBaoHead.totailOrderWeight}}</span>
              </el-row>
            </div>
            <div>
                    ...
                    <div class="pro-item iitem-left">
                      ...
                      <div class="pro-right">
                        ...
                        <el-row>
                          ...
                          <el-col :span="4" class="pro-middle txtFour">应收数量：{{item.oriSkuSum}}</el-col>
                        ...
                        <el-row>
                          ...
                          <el-col :span="4" class="pro-middle">
                            <span v-if="!item.suitTF"><span v-if="item.carFlower">车花：{{item.carFlower}}</span></span>&nbsp;
                          </el-col>
                          <el-col
                            :span="4"
                            class="pro-middle txtFour"
                          >应收净重(g)：{{item.skuWeight?item.skuWeight:(item.standardGold*item.oriSkuSum)}}</el-col>
                          <el-col :span="6" class="pro-middle txtFour txtRed">
                            实收净重(g)：
                            <el-input-number
                              :controls="false"
                              v-model="item.editSkuGoldWeight"
                              v-if="item.editSku"
                              size="small"
                              class="width70"
                            ></el-input-number>
                            <span
                              v-else
                            >{{item.nskuGoldWeight?item.nskuGoldWeight:item.editSkuGoldWeight}}</span>
                        ...
                        <el-row>
                          ...
                          <el-col :span="13" class="pro-middle txtFour">
                            <span
                              v-if="!item.suitTF"
                          ...
                          <el-col :span="6" class="pro-middle txtFour txtRed">
                            ...
                            <el-input-number
                              ...
                              class="width70"
                            ></el-input-number>
                            <span v-else>{{item.nskuWeight?item.nskuWeight:item.editSkuWeight}}</span>
                          </el-col>
                        </el-row>
                      </div>
                    </div>
                          ...
                          <div class="tabThreeRight">
                            <el-row>
                              ...
                              <el-col
                                :span="10"
                                class="pro-middle-Two"
                              >克重(g)：{{jelem.standardGold?jelem.standardGold:jelem.nskuGoldWeight}}</el-col>
                ...
                <div class="pro-item iitem-left">
                  ...
                  <div class="pro-right">
                    ...
                    <el-row style="height: 35px;">
                      ...
                      <el-col :span="4" class="pro-middle txtFour">应收数量：{{iitem.oriSkuSum}}</el-col>
                    ...
                    <el-row style="height: 35px;">
                      ...
                      <el-col :span="4" class="pro-middle txtFour">应收净重(g)：{{iitem.goldWeight}}</el-col>
                      <el-col :span="6" class="pro-middle redTxtOne">
                        实收净重(g)：
                        <span v-if="iitem.skuEditTF || !editTF">{{iitem.editSkuGoldWeight}}</span>
                        <el-input-number
                          v-else
                          v-model="iitem.editSkuGoldWeight"
                          class="width70"
                          :controls="false"
    ...
    <div ref="printTabTwo" v-show="false" style="width:200px">
      ...
      >
        ...
        <div>净重：{{viewBaoHead.editSkuGoldWeight}}</div>
        <div>毛重：{{viewBaoHead.editSkuWeight}}</div>
      </div>
    </div>
  </div>
</template>
...
import $filter from "./acommon/filter.js";
export default {
  data() {
    return {
      stepsActive: 1,
  ...
  methods: {
    created_fun() {
      ...
      this.$api.formdataPostFun(
        ...
        res => {
          ...
          self.orderList = $filter.setDatasource(res.data);
          // console.log(self.orderList)
          self.orderList.index = 1;
          // if(self.orderList.receiptOrderProducts){
    ...
    delSkuFun(elem, index) {
      ...
      if (!newTF) {
        ...
        tempObj.editSkuGoldWeight +=
          self.viewBaoHead.receiptOrderProducts[index].editSkuGoldWeight;
    ...
    baoNumFun(ielem) {
      ...
      if (ielem.receiptOrderProducts) {
        ielem.receiptOrderProducts.forEach(jelem => {
          ...
          if (jelem.editSkuGoldWeight) {
            skuGoldWeightNum += parseFloat(jelem.editSkuGoldWeight);
          } else {
            // pickWeightNum += parseFloat(jelem.standardGold * jelem.oriSkuSum);
          }
        });
        ielem.orderSum = baoAmountNum;
        ielem.totailGoldWeight = self.$api.returnFloat(skuGoldWeightNum);
    ...
    baoNumTwoFun() {
      ...
      let pickWeightNum = 0;
      let skuGoldWeightNum = 0;
      // console.log(self.viewBaoHead)
      self.viewBaoHead.receiptOrderProducts.forEach(jelem => {
        // console.log(ielem)
        // if (ielem.receiptOrderProducts) {
        //   ielem.receiptOrderProducts.forEach(jelem => {
        if (jelem.editSkuSum) {
      ...
      self.viewBaoHead.orderSum = baoAmountNum;
      self.viewBaoHead.totailGoldWeight = self.$api.returnFloat(
        skuGoldWeightNum
      );
      self.viewBaoHead.totailOrderWeight = self.$api.returnFloat(pickWeightNum);
    },
    addBackup(elem) {
      let self = this;
    ...
    confirmBaoFun(elem) {
      ...
      elem.receiptOrderProducts.forEach(jelem => {
        receiptPackInfos.push({
          ...
          nowSkuGoldWeight: jelem.editSkuGoldWeight,
    ...
    changeWeight(elem){
      ...
      self.orderList.receiptOrderProducts[indexOne].editSkuWeight = parseFloat(elem);
      self.$forceUpdate();
    },
    //循环读取电子称输入框数量
    loop() {
