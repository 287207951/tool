    ...
    <div>
      ...
      <el-row>
        <el-col :span="19">
          ...
          <!-- <span class="fontStyle">调拨来源</span>
          <el-select
            v-model="orderTypeId"
            placeholder="请选择"
            size="small"
        ...
        <el-col :span="5">
          <!-- <el-button type="primary" size="small" @click="excelInputFun" :loading="xlsLoadTF">导入EXCEL</el-button> -->
          <input
            type="file"
            ref="excelInput"
            accept=".xls, .xlsx"
      ...
      >
        ...
        <el-table-column prop="supplierfName" label="发起单位">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.sponsor == 'D'">尚金缘展厅</span>
          </template>
        </el-table-column>
        <el-table-column prop="orderType" label="接收客户">
          <template slot-scope="scope">
            <span v-if="scope.row.putType == undefined">-</span>
            <span v-if="scope.row.putType == '1'">尚金缘展厅</span>
            <span v-if="scope.row.putType == '2'">尚金缘金库</span>
          </template>
        </el-table-column>
        <el-table-column prop="orderTotalSum" label="调拨数量"></el-table-column>
        <!-- <el-table-column
          prop="totailOrderWeight"
          label="毛重(g)"
        ></el-table-column>-->
        <el-table-column prop="totailGoldWeight" label="净重(g)"></el-table-column>

        <el-table-column prop="orderStatus" label="订单状态">
          <template slot-scope="scope">
            <span v-if="scope.row.orderStatus == undefined">-</span>
            <span v-if="scope.row.orderStatus == '1'" class="yellowColor">待签收</span>
            <span v-if="scope.row.orderStatus == '2'" class="blackColor">已通过</span>
            <span v-if="scope.row.orderStatus == '4'" class="redColor">已驳回</span>
            <span v-if="scope.row.orderStatus == '6'" class="blackColor">已签收</span>
            <span v-if="scope.row.orderStatus == '11'" class="redColor">已作废</span>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    displayAllData(num, size) {
      ...
      this.$api.formdataPostFun(
        this.$portMain + "/order/selectOpretaOrderGoldTrandf",
        params,
        res => {
          if (res.code == 200) {
