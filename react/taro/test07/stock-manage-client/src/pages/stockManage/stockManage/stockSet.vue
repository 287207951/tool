    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="type" label="仓库类型">
          <template slot-scope="scope">{{getStockType(scope.row.type)}}</template>
        </el-table-column>
        <el-table-column prop="secName" label="所属部门"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取仓库类型中文
    getStockType(option) {
      return this.stockTypeList.find(item => item.val === option).txt;
    },
