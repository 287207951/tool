<template>
  <div class="conBigDiv" ref="materialTransfer">
    ...
    <div>
      ...
      <el-row>
        <el-col :span="12">
          <el-button type="primary" size="small" @click="createdFun">新建转饰</el-button>
          <el-button type="danger" size="small" plain @click="queryFun">查询</el-button>
        </el-col>
        <el-col :span="12" class="totalDes" v-show="false">
          <span>总计：数量{{firstWeight}}</span>
          <span>重量{{finalWeight}}g</span>
          <span>折重{{totalMoney}}g</span>
        </el-col>
      </el-row>
      <el-table
        :data="allData"
      ...
      >
        <el-table-column prop="to_ornament_code" label="转饰单号"></el-table-column>
        <el-table-column prop="company_name" label="公司"></el-table-column>
        <el-table-column prop="customer_name" label="入库柜"></el-table-column>
...
export default {
  ...
  created() {
    ...
    document.onkeydown = e => {
      ...
      let dom = this.$refs.materialTransfer;
      ...
      this.$axios
        ...
        .then(function(res) {
          if (res.data.code == 200) {
            ...
            self.totalMoney = res.data.data.totalMoney;
