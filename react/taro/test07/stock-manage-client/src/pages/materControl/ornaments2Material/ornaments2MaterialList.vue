<template>
  <div class="conBigDiv"
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right"
      ...
      <el-breadcrumb-item>饰转料(入库)</el-breadcrumb-item>
    </el-breadcrumb>
    <div>
      <div class="hr"></div>
      <el-row>
        <el-col :span="24">
          ...
          <el-select v-model="searchForm.invoiceStatu"
                     placeholder="请选择"
                     size="small"
                     style="width:200px"
                     @change="screenFun">
            <el-option v-for="item in invoiceStatuOptions"
                       :key="item.code"
                       :label="item.name"
      ...
      <el-table :data="tableData"
        ...
        <el-table-column prop="recipientRepoName"
                         label="入库柜">
        </el-table-column>
...
export default {
  ...
  methods: {
    new2M () {
      this.$router.push({ path: "o2MDetail" })
    },
    // 获取出库状态options
    getInvoiceStatuOptions () {
      let params = {
        type: "JEWELRY_MATERIAL_END"
      };
    ...
    searTimechFun () {
      if (this.selectedDay != null) {
        this.searchForm.dateStart = this.$api.dateGetDayTime(this.selectedDay[0]);
    ...
    tableRender () {
      ...
      this.$axios
        .get(this.$portMain + "/convert/mlist", params)
        .then(function (res) {
          if (res.data.code == 200) {
