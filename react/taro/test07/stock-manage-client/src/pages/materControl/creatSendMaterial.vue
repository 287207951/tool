  ...
  <div class="conBigDiv" ref="creatSendMaterial">
    ...
    <div>
      ...
      <el-row class="itemFirst">
        ...
        <el-col :span="8">
          <div>
            <span style="margin-left: 13px;">收发柜</span>
            <el-select
      ...
      <div style="display: flex;">
        ...
        >
          <el-table-column label="出货明细" header-align="center">
            ...
            <el-table-column prop="output_fur_weight" label="显示重量(g)">
              <template slot-scope="scope">
                <el-input
                  v-model="scope.row.output_fur_weight"
                  placeholder="请输入内容"
                  size="small"
                  @blur="checkWeight(scope.row)"
...
export default {
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(function(res) {
          if (res.data.code == 200) {
            ...
            if (
              self.findPositionData != undefined &&
              self.findPositionData.length > 0
            ) {
              self.submitData.counter_id = self.findPositionData[0].id;
              self.submitData.output_counter =
                self.findPositionData[0].enter_counter;
            }
          } else {
            self.$message.error(res.data.msg);
          }
      ...
      this.$axios
        ...
        .then(function(res) {
          if (res.data.code == 200) {
            ...
            if (
              self.stockSetKindData != undefined &&
              self.stockSetKindData.length > 0
            ) {
              self.submitData.stock_kind_id = self.stockSetKindData[0].id;
              self.submitData.stock_kind =
                self.stockSetKindData[0].material_name;
            }
          } else {
            self.$message.error(res.data.msg);
          }
    ...
    submitPrint() {
      ...
      let result2 = false;
      this.submitData.creditLines.forEach(item => {
        if (
          item.expense_way_id == "" ||
          item.project == "" ||
          item.k_weight == "" ||
        ...
        res => {
          if (res.code == 200) {
            ...
            let output_code = res.data.output_code
              ...
              : "--";
            self.modifyPrintData(paramData, output_code);
            setTimeout(function() {
    ...
    modifyPrintData(param, output_code) {
      let self = this;
      this.printData = JSON.parse(param.jsonObjs);
      this.printData.output_code = output_code;
      this.printData.update_time = new Date();
    ...
    //成色校验
    checkCondition(elem) {
      if (elem.output_quality != undefined) {
        elem.output_quality = elem.output_quality.trim();
      }

      let num = elem.output_quality;
      let result = isNaN(num);
      let result2 = Number(num) < 0;

      if (result || result2) {
        this.$message.error("请输入有效数据！");
        elem.output_quality = 0;
        elem.output_discount_weight = 0;
        return;
      }
    ...
    //毛重校验
    checkWeight(elem) {
      if (elem.output_fur_weight != undefined) {
        elem.output_fur_weight = elem.output_fur_weight.trim();
      }

      let result = isNaN(elem.output_fur_weight);
      let result2 = Number(elem.output_fur_weight) < 0;
      if (result || result2) {
        this.$message.error("请输入有效数据!");
        elem.output_fur_weight = 0;
        elem.output_discount_weight = 0;
      }
    },
    //去空
