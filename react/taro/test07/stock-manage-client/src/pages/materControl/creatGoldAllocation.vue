<template>
  <div class="conBigDiv customTable" ref="creatGoldAllocation">
    ...
    <div>
      ...
      <el-row>
        <el-col :span="24" class="itemRight">
          ...
          <el-button
            type="primary"
            size="small"
            @click="canSubmitPrintStatus && submitPrint()"
      ...
      >
        ...
        <el-table-column header-align="center" align="center" prop="k_weight" label="重量(g)">
          <template slot-scope="scope">
            <el-input
              v-model="scope.row.k_weight"
              placeholder="请输入内容"
              size="small"
              @blur="comptMoney(scope.row,'weight')"
            ></el-input>
          </template>
        </el-table-column>
        <el-table-column header-align="center" align="center" prop label="克工费">
          <template slot-scope="scope">
            <el-input
              ...
              placeholder="请输入内容"
              size="small"
              :disabled="scope.row.p_wage !== ''"
              @blur="comptMoney(scope.row,'feeG')"
            ></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column header-align="center" align="center" prop="p_amount" label="件数">
          <template slot-scope="scope">
            <el-input
              ...
              placeholder="请输入内容"
              size="small"
              @blur="comptMoney(scope.row,'number')"
            ></el-input>
          </template>
        </el-table-column>
        <el-table-column header-align="center" align="center" prop label="件工费">
          <template slot-scope="scope">
            <el-input
              ...
              placeholder="请输入内容"
              size="small"
              :disabled="scope.row.k_wage !== ''"
              @blur="comptMoney(scope.row,'feeJ')"
            ></el-input>
          </template>
        </el-table-column>
    ...
    <!-- 打印 -->
    <div v-show="false">
      <PrintTPEight :billData="printData" ref="myComp"></PrintTPEight>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      ...
      priceSum: 0, //总条目数
      canSubmitPrintStatus: true // 可以提交打印状态
    };
  },
  created() {
    this.created_fun();
    ...
    document.onkeydown = e => {
      ...
      let dom = this.$refs.creatGoldAllocation;
  ...
  methods: {
    ...
    comptMoney(item, proct) {
      ...
      if (proct == "feeG") {
        if (item.k_wage != undefined) {
          item.k_wage = item.k_wage.trim();
        }

        let result = isNaN(item.k_wage);
        let result2 = Number(item.k_wage) < 0;
        if (result || result2) {
          this.$message.warning("请输入有效数字！");
          item.k_wage = "";
          return;
      ...
      if (proct == "number") {
        if (item.p_amount != undefined) {
          item.p_amount = item.p_amount.trim();
        }

        let result = isNaN(item.p_amount);
        let result2 = Number(item.p_amount) < 0;
        if (result || result2) {
          this.$message.warning("请输入有效数字！");
          item.p_amount = "";
          return;
        ...
        let result3 = Number(item.p_amount) % 1 === 0;
        if (!result3) {
          this.$message.warning("请输入有效数字！");
          item.p_amount = "";
          return;
      ...
      if (proct == "feeJ") {
        if (item.p_wage != undefined) {
          item.p_wage = item.p_wage.trim();
        }

        let result = isNaN(item.p_wage);
        let result2 = Number(item.p_wage) < 0;
        if (result || result2) {
          this.$message.warning("请输入有效数字！");
          item.p_wage = "";
          return;
      ...
      Number(item.money) !== 0 && (item.money = Math.round(item.money));
    },
    // 新增一行
    addFun() {
    ...
    submitPrint() {
      ...
      if (this.departId == "" || this.departId == undefined) {
        self.$message.error("请选择调拨单位！");
        return;
      }
      if (
      ...
      if (result) {
        self.$message.error("请填写表单里面的重量！");
        return;
      }

      this.canSubmitPrintStatus = false;
      let paramData = {
      ...
      self.$api.formdataPostFun(
        self.$portMain + "/stock/addAllotOrder",
        paramData,
        res => {
          if (res.code == 200) {
            self.$message.success(res.msg);
            self.submitData.allotOrderLines.forEach(item => {
              item.stock_type = self.submitData.stock_type;
            });
            self.printData = self.submitData;
            self.printData.allot_code = res.data.allot_code
              ? res.data.allot_code
              : "--";
            self.printData.recestock_depart_name = "金库收料部";
        ...
        err => {
          ...
          this.canSubmitPrintStatus = true;
          self.btnLoadingType = false;
        }
      );
    },
    ...
    //去空
    goEmpty(item) {
      if (item.p_standard != undefined) {
        item.p_standard = item.p_standard.trim();
      }
