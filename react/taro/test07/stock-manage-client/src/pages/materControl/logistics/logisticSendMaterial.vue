<template>
  <div class="conBigDiv" ref="logisticSendMaterial" v-loading="loading">
    ...
    <div>
      ...
      >
        <el-table-column prop="output_code" label="发料单号" width="120"></el-table-column>
        <el-table-column prop="depart_name" label="发料单位"></el-table-column>
        <el-table-column prop="customer_name" label="收料单位" width="120"></el-table-column>
        <el-table-column prop="customer_type" label="类型"></el-table-column>
        ...
        <el-table-column prop="bills_statue" label="单据状态" width="80">
          <template slot-scope="scope">
...
export default {
  ...
  created() {
    ...
    document.onkeydown = e => {
      ...
      let dom = this.$refs.logisticSendMaterial;
  ...
  methods: {
    ...
    displayAllData(num, size) {
      ...
      this.$axios
        .get(this.$portMain + "/stock/logistics/NoutputStockList", params)
        .then(function(res) {
    ...
    editGold(elem) {
      this.$router.push({
        path: "logisticSendMaterialDetail",
    ...
    // 新建发料
    createdFun() {
      this.$router.push("logisticCreatSendMaterial");
    },
    // 查询金料种类
    findStockSetKind() {
      let self = this;
    ...
    resetQueryFun() {
      this.search = this.startTime = this.endTime = this.customerType = this.stockKindId = this.stockTypeId = this.outputStatue = this.billsStatue = this.selectedDay =
        "";
      this.queryFun();
    },
    //权限判断
