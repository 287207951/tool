<template>
  <div class="conBigDiv customTable" ref="logisticCreatGoldAllocation">
        ...
        <el-table-column header-align="center" align="center" prop label="克工费">
          <template slot-scope="scope">
            <el-input
              ...
              placeholder="请输入内容"
              size="small"
              @blur="comptMoney(scope.row,'k_wage')"
            ></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column header-align="center" align="center" prop="p_amount" label="件数">
          <template slot-scope="scope">
            <el-input
              ...
              placeholder="请输入内容"
              size="small"
              @blur="comptMoney(scope.row,'p_amount')"
            ></el-input>
          </template>
        </el-table-column>
        <el-table-column header-align="center" align="center" prop label="件工费">
          <template slot-scope="scope">
            <el-input
              ...
              placeholder="请输入内容"
              size="small"
              @blur="comptMoney(scope.row,'p_wage')"
            ></el-input>
          </template>
        </el-table-column>
...
export default {
  ...
  created() {
    ...
    document.onkeydown = e => {
      ...
      let dom = this.$refs.logisticCreatGoldAllocation;
  ...
  methods: {
    ...
    queryFun() {
      //
      this.$router.push("logisticGoldAllocation");
    },
    // 查询金料种类
    findStockSetKind() {
      let self = this;
    ...
    comptMoney(item, name) {
      ...
      if (item.k_wage != "" && item.k_weight != "") {
        item.money = (item.k_wage * item.k_weight).toFixed(2);
      } else if (item.p_amount != "" && item.p_wage != "") {
        item.money = (item.p_amount * item.p_wage).toFixed(2);
      } else {
        item.money = "";
      }
    ...
    submitPrint() {
      ...
      self.$api.formdataPostFun(
        self.$portMain + "/stock/logistics/addAllotOrder",
        paramData,
        res => {
          if (res.code == 200) {
            ...
            self.printData.allot_code = res.data.allot_code
              ? res.data.allot_code
              : "--";
            self.printData.recestock_depart_name = "物流中心";
            self.printData.allotOrderLines.forEach(item => {
              item.stock_type = self.printData.stock_type;
            });
            self.$refs.myComp.compPrintFun();
            setTimeout(function() {
              self.btnLoadingType = false;
              self.$router.push("logisticGoldAllocation");
            }, 2000);
          }
        },
        err => {
