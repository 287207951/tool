<template>
  <div class="conBigDiv" ref="logisticGoldAllocation" v-loading="loading">
    ...
    <div>
      ...
      >
        <el-table-column prop="allot_code" label="调拨单号" width="140"></el-table-column>
        <el-table-column prop="recestock_depart_name" label="调拨单位" width="120"></el-table-column>
        <el-table-column prop="customer_name" label="接收单位" width="120"></el-table-column>
        ...
        <el-table-column label="出库状态">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.output_statue == 2">已签收</span>
            <span v-if="scope.row.output_statue == 3">已驳回</span>
            <span v-if="scope.row.output_statue == 4">作废</span>
          </template>
        </el-table-column>
        <el-table-column prop="note" label="备注信息"></el-table-column>
        <el-table-column prop="update_time" label="操作时间" width="160"></el-table-column>
...
export default {
  ...
  created() {
    ...
    document.onkeydown = e => {
      ...
      let dom = this.$refs.logisticGoldAllocation;
  ...
  methods: {
    ...
    displayAllData(num, size) {
      ...
      this.$axios
        .get(this.$portMain + "/stock/logistics/allotOrderList", params)
        .then(function(res) {
    ...
    editGold(elem) {
      this.$router.push({
        path: "logisticGoldAllocationDetail",
    ...
    // 新建发料
    createdFun() {
      this.$router.push("logisticCreatGoldAllocation");
    },
    // 查询金料种类
    findStockSetKind() {
      let self = this;
