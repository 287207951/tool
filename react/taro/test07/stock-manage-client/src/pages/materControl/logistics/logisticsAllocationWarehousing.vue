  ...
  <div class="conBigDiv" v-loading="loading">
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="company_name" label="调拨单位"></el-table-column>
        ...
        <el-table-column label="单据状态">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.bills_statue == 2" class="redColor">审核通过</span>
            <span v-if="scope.row.bills_statue == 3" class="redColor">审核驳回</span>
          </template>
        </el-table-column>
        <el-table-column prop label="备注信息">
          <template slot-scope="scope">
            <span class="noteStyle">{{scope.row.note}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template slot-scope="scope">
            <div v-if="scope.row.output_statue == 1 && powerBtnShow == 'true'">
            ...
            <div
              v-if="scope.row.output_statue == 2 && scope.row.bills_statue == 2 && powerBtnShow == 'true'"
            >
              <el-button
                type="primary"
                size="mini"
    ...
    <el-dialog
      title="审核入库"
      :visible.sync="findPositionVisible"
      width="480px"
      :close-on-click-modal="false"
    >
      <div class="promptStyle">
...
export default {
  ...
  methods: {
    ...
    displayAllData(num, size) {
      ...
      this.$axios
        .get(this.$portMain + "/stock/logistics/allotInputList", params)
        .then(function(res) {
    ...
    editGold(elem) {
      this.$router.push({
        path: "logisticsAllocationWarehousingDetail",
    ...
    resetQueryFun() {
      this.search = this.startTime = this.endTime = this.stockKindId = this.stockTypeId = this.outputStatue = this.billsStatue = this.selectedDay =
        "";
      this.queryFun();
    },
    // 新建发料
    ...
    printFun(item) {
      ...
      this.$axios
        .get(this.$portMain + "/stock/logistics/printAllotInput", params)
        .then(function(res) {
          if (res.data.code == 200) {
            self.$message.success(res.data.msg);
            self.detailData(item.id);
            self.created_fun();
          } else {
            self.$message.error(res.data.msg);
          }
    ...
    passFun() {
      ...
      this.$axios
        .get(this.$portMain + "/stock/logistics/passAllotInput", params)
        .then(function(res) {
          if (res.data.code == 200) {
            self.$message.success(res.data.msg);
    ...
    rejectFun(item) {
      ...
      this.$axios
        .get(this.$portMain + "/stock/logistics/rejectAllotInput", params)
        .then(function(res) {
          if (res.data.code == 200) {
            self.$message.success(res.data.msg);
            self.created_fun();
    ...
    detailData(orderId) {
      ...
      let params = {
        PRS: {
          allotOrderId: orderId
        }
      };
      this.$axios
        .get(this.$portMain + "/stock/logistics/allotInputFrom", params)
        .then(function(res) {
          if (res.data.code == 200) {
            self.printData = res.data.data;
