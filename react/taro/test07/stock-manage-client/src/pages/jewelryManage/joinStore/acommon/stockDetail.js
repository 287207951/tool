...
class StockDetail {
    ...
    // 选项
    initWayData(options) {
        this.wayData = options.filter(item => {
            return item.ceditWayId !== 2 && item.ceditWayId !== 22;
        });
        return this.wayData
    }
    ...
    isPriceAddBackDiscount(options, index) {
        // console.log(this.list[index + 1], options, index)
        // console.log(this.list[index + 1].ceditWayId === 8)
        // console.log(options.backId === this.list[index + 1].backId)
        return this.list[index + 1] && this.list[index + 1].ceditWayId === 8 && (options.backId && options.backId === this.list[index + 1].backId)
    }
    ...
    getProStockDetailParams() {
        ...
        arr.forEach((item, index) => {
            ...
            if (item.ceditWayId === 1 && this.isPriceAddBackDiscount(item, index)) {
                // console.log(this.isPriceAddBackDiscount(item, index))
                // 工费加收退工费改变标识
