    ...
    <div class="product">
      ...
      <div style="display:flex;flex:6">
        <div class="uLTwo" style="flex:8">
          <div>
            <span class="grayCol">转料数量：</span>
            <span>{{productData.skuNum}}</span>
          </div>
          <div>
            <span class="grayCol">转料净重：</span>
            <span>{{productData.editSuttle}}g</span>
          </div>
          <div>
            <span class="grayCol">转料毛重：</span>
            <span>{{productData.editWeight}}g</span>
          </div>
        </div>
        <div class="uLThree" style="flex:10">
          <div>
            <span class="grayCol">出库原因：</span>
            <span v-if="(typeof productData.resion) != 'string'">
              <el-popover
                placement="top-start"
                width="200"
                trigger="hover"
                :content="productData.resion.join()"
              >
                <span
                  class="txtBlack overflowHidden"
                  slot="reference"
                >{{productData.resion.join().length < 10?productData.resion.join():productData.resion.join().substr(0, 10)+'...'}}</span>
              </el-popover>
            </span>
            <span v-else>{{productData.resion}}</span>
          </div>
          <div>
