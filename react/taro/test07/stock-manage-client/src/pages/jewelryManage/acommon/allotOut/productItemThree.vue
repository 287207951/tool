    ...
    <div class="product">
      ...
      <div style="display:flex;flex:6">
        <div class="uLTwo" style="flex:6">
          <div>
            <span class="grayCol">{{invoiceType === 'STOCK_IN'?'入库':'调拨'}}数量：</span>
            <span>{{productData.amount}}</span>
          </div>
          <div>
            <span class="grayCol">{{invoiceType === 'STOCK_IN'?'入库':'调拨'}}净重：</span>
            <span>{{productData.suttle}}g</span>
          </div>
          <div>
            <span class="grayCol">{{invoiceType === 'STOCK_IN'?'入库':'调拨'}}毛重：</span>
        ...
        <div class="uLThree" style="flex:6">
          <div v-if="productData.remarkReason">
            <span class="grayCol">{{invoiceType === 'STOCK_IN'?'入库':'调拨'}}原因：</span>
            <span>
              <el-popover
                placement="top-start"
                width="200"
          ...
          <div v-if="productData.remark">
            <span class="grayCol">{{invoiceType === 'STOCK_IN'?'入库':'调拨'}}备注：</span>
            <span>
              <el-popover
                placement="top-start"
                width="200"
...
export default {
  ...
  methods: {
    ...
    focusWeightFocus(e, elem) {
      ...
      localStorage.indOne = this.indOne;
      localStorage.tempObj = JSON.stringify(e);
    },
    //输入框丢去焦点
    focusWeightBlur(item, name) {
      let self = this;
      localStorage.tempObj = null;
      localStorage.indOne = null;
      localStorage.enterStatus = null;
      this.enterStatus = "";
      if (name == "newSuttle") {
    ...
    comFun() {
      var self = this;
      let tempObj = JSON.parse(localStorage.tempObj);
    ...
    changeWeight(elem) {
      let self = this;
      let enterStatus = localStorage.enterStatus;
      let indOne = localStorage.indOne;
      if (enterStatus === "复称净重") {
        this.proList[indOne].newSuttle = parseFloat(elem);
      }
      if (enterStatus === "复称毛重") {
        this.proList[indOne].newWeight = parseFloat(elem);
      }
      self.$forceUpdate();
    },
    //循环读取电子称输入框数量
