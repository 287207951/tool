  ...
  <div>
    ...
    <section class="infoWrap">
      <div style="display:flex;margin-bottom: 36px;">
        ...
        <div class="btn" style="flex:1" v-if="allData.bizInvoice.invoiceStatu != 'EXREPO'">
          <div class="btnItem">
      ...
      <RadioComponents
        :timeStamp="timeStamp"
        :controlSelectWay="controlSelectWay"
        :shopData="shopData"
      ...
      <CreatedOutOfStockDetail
        ...
        :SelectedShopPackId="SelectedShopPackId"
        @printFun="printFun"
      ></CreatedOutOfStockDetail>
  ...
  methods: {
    ...
    displayAllData() {
      ...
      this.$api.formdataPostFun(
        this.$portMain + "/outStockBills/findDataClient",
        params,
        res => {
    ...
    assemblyPackageData() {
        ...
        parentPack.forEach(one => {
          bizPack.forEach(two => {
            if (one.packId == two.supPackId) {
              one.childPack.push(
                Object.assign(two, { shopNameRemarks: one.shopName }) //将店名放入子包里面
              );
            }
          });
        });
      ...
      if (this.allData.parentPack != undefined) {
        //有店铺
        this.assembleShopData();
        //默认第一个店铺的成色
        this.shopChildPacksData = this.allData.parentPack[0].childPack;
        this.assembleConditionData(this.allData.parentPack[0].childPack);
        // console.log("组装数据", this.allData);
      } else {
        //没店铺
        this.shopShow = false; //门店显示状态
        this.wayShow = false; //打单方式显示
        this.assembleConditionData(this.allData.bizPack);
      }
      // console.log("组装完成", this.allData);
    },
    //组装门店数据
    assembleShopData() {
      if (
      ...
      // console.log("组装数据", shopData);
      this.originalShopData = JSON.parse(JSON.stringify(shopData));
      this.SelectedShopPackId = shopData[0].packId;
      this.shopName = shopData[0].shopName;
    },
    //组装成色数据
    ...
    //选择门店
    shopFun(val) {
      // console.log("选择门店11", val);
      this.SelectedShopPackId = val;
      let childPack = [];
    ...
    conditionFun(val) {
      let childPacksData = [],
        packageData = [];
      if (this.allData.parentPack != undefined) {
        packageData = this.shopChildPacksData;
      } else {
        packageData = this.allData.bizPack;
      }
    ...
    temporaryFun() {
      ...
      this.$confirm("确定要暂存这个包么?", "提示", {
        ...
        .then(() => {
          let packList = [];
          self.selectPackageData.forEach(item => {
          ...
          self.$api.formdataPostFun(
            self.$portMain + "/outStockBills/updateClientTS",
            params,
            res => {
    ...
    cancelStorageFun(val) {
      ...
      this.$api.formdataPostFun(
        this.$portMain + "/outStockBills/cancelClientTS",
        params,
        res => {
          self.$message.success(res.msg);
          self.displayAllData();
    ...
    confirmFun() {
      ...
      //   //将选中的店铺名放到出货明细的备注里面
      //   item.shopNameRemark = this.shopName;
      // });
      this.dialogType = new Date().getTime();
      // console.log("勾选产品", this.selectPackageData);
    },
    //返柜
    returnCabinetFun() {
      ...
      this.$confirm("请确定是否要返柜?", "提示", {
        ...
        .then(() => {
          ...
          // return;
          self.$api.formdataPostFun(
            self.$portMain + "/outStockBills/returnCabinetClient",
            params,
            res => {
              self.$message.success(res.msg);
              self.displayAllData();
    ...
    selectPackageFun() {
      ...
      // console.log("选择的id", this.selectPackageData);
    },
    //全选事件
    allSelectFun() {
      this.selectPackageData = [];
    ...
    getMyInfo() {
      let self = this;
      this.$axios.get(this.$portMain + "/account/myinfo").then(function(res) {
        if (res.data.code == 200) {
          self.userData = res.data.data;
          console.log("用户信息", self.userData);
        }
      });
    },
    printFun(printData) {
      ...
      this.printData.creator =
        this.allData.bizInvoice.creator === "系统"
          ? this.userData.concat
          : this.allData.bizInvoice.creator;
